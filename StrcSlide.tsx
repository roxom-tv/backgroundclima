...
// ── Config ──
// Change this to match wherever the STRC Express server is running
const STRC_API_URL = process.env.NEXT_PUBLIC_STRC_API_URL || 'http://192.168.1.14:3001';
...
async function fetchStrcInternal(
  ...
  fetchPromise = (async () => {
    ...
    setLoading?.(true);
    try {
      const res = await fetch(`${STRC_API_URL}/api/data`, { cache: 'no-store' });
