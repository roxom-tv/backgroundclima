...
export default function StrcSlide() {
    ...
    if (!data || error) {
        return (
            ...
            >
                <div className="text-red-500 text-xl font-mono">Error loading STRC data</div>
            </motion.div>
        );
    }
    ...
    const atmSats = isActive ? todayDiv!.sats : toSats(metrics.monthlyDiv, b);
    ...
    const stats = [
        ...
        {
            l: 'Monthly Div',
            v: fmtSats(metrics.monthlyDivSats) + ' sats',
            u: fmtUSD(metrics.monthlyDiv, 4),
            gold: true,
        },
        {
            l: 'Annual Div',
            v: fmtSats(metrics.annualDivSats) + ' sats',
            u: fmtUSD(metrics.annualDiv, 4),
            gold: true,
        },
        {
            l: 'Market Cap',
            v: (metrics.marketCap / b).toFixed(0) + ' BTC',
            u: fmtUSD(metrics.marketCap, 0),
        },
        { l: 'Volume', v: strc.volume.toLocaleString('en-US') },
        {
            l: 'Prev Close',
            v: fmtSats(toSats(strc.previousClose, b)) + ' sats',
            u: fmtUSD(strc.previousClose),
        },
            ...
            >
                <div className="flex items-center gap-6">
                    ...
                    <div className="flex items-baseline gap-4">
                        ...
                        >
                            {sign}
                            {diff.toLocaleString('en-US')} sats
                        </span>
                    </div>
                </div>
                <div className="flex items-center gap-4">
                    ...
                    >
                        BTC{' '}
                        <span style={{ color: '#CBD5E1', fontWeight: 600 }}>{fmtUSD(b, 0)}</span>
                    </span>
                    <div
                        style={{
                            width: 8,
