        ...
        <div className={`admin-stat ${borderColors[color]}`}>
            <div className="flex items-start justify-between">
                <div>
                    ...
                    <p className="admin-stat-value">{isLoading ? '---' : value}</p>
...
export const QuickAction = memo(function QuickAction({
    ...
    return (
        <Link href={href} className="admin-card block transition-colors hover:bg-[#1a1a1a]">
            <div className="flex items-center gap-3">
                <span className="text-2xl opacity-70">{icon}</span>
                <div>
                    <h3 className="text-white font-mono font-semibold text-sm uppercase tracking-wider">
                        {title}
                    </h3>
...
export const PageHeader = memo(function PageHeader({ title, subtitle, action }: PageHeaderProps) {
    return (
...
export const Notification = memo(function Notification({ type, message }: NotificationProps) {
    return (
...
export const InfoBox = memo(function InfoBox({ title, children, icon = 'i' }: InfoBoxProps) {
    return (
...
export const ToggleSwitch = memo(function ToggleSwitch({
    ...
    return (
        <button onClick={onChange} className={`admin-toggle ${checked ? colors[color] : ''}`}>
            <div className={`admin-toggle-knob ${checked ? 'translate-x-7' : 'translate-x-0'}`} />
        </button>
    );
});
