...
async function fetchChart(symbol: string): Promise<ChartRaw> {
    ...
    if (!res.ok) {
        throw new Error(`${symbol} chart HTTP ${res.status}`);
    }
...
async function getYahooCrumb(): Promise<{ crumb: string; cookie: string }> {
    ...
    if (!r2.ok) {
        throw new Error(`crumb HTTP ${r2.status}`);
    }
...
async function fetchQuotesBatch(symbols: string[]): Promise<Record<string, QuoteRaw>> {
    ...
    if (!res.ok) {
        throw new Error(`quote batch HTTP ${res.status}`);
    }
    const json = await res.json();
    ...
    for (const row of rows) {
        ...
        out[symbol] = {
            ...
            eps:
                typeof row.epsTrailingTwelveMonths === 'number'
                    ? row.epsTrailingTwelveMonths
                    : null,
            beta: typeof row.beta === 'number' ? row.beta : null,
            avgVol:
                typeof row.averageDailyVolume3Month === 'number'
                    ? row.averageDailyVolume3Month
                    : typeof row.averageDailyVolume10Day === 'number'
...
async function fetchSummary(symbol: string): Promise<SummaryRaw> {
    ...
    if (!res.ok) {
        throw new Error(`${symbol} summary HTTP ${res.status}`);
    }
    const json = await res.json();
    const r = json.quoteSummary?.result?.[0];

    if (!r) {
        return { pe: null, eps: null, beta: null, avgVol: null, mktCap: null };
    }
    ...
    try {
        ...
        WATCHLIST.forEach((sym, i) => {
            if (chartResults[i].status === 'fulfilled') {
                charts[sym] = (chartResults[i] as PromiseFulfilledResult<ChartRaw>).value;
            }
        });
        ...
        INDEX_POOL.forEach(({ sym }, i) => {
            if (idxResults[i].status === 'fulfilled') {
                idxRaw[sym] = (idxResults[i] as PromiseFulfilledResult<ChartRaw>).value;
            }
        });
        ...
        const provisionalTopSymbols = WATCHLIST.filter((sym) => charts[sym])
            .map((sym) => {
                const c = charts[sym];
                const price = c.price;
        ...
        const missingOrIncompleteSymbols = provisionalTopSymbols.filter(
            (sym) => !hasCompleteSummary(staleCache[sym]),
        );
        ...
        // 5. Merge & derive
        const allRankedTickers = WATCHLIST.filter((sym) => charts[sym]).map((sym) => {
            const c = charts[sym];
            ...
            const w52Pos =
                c.w52Low && c.w52High && c.w52High > c.w52Low
            ...
            if (Math.abs(chgPct) >= 3) {
                badges.push('move');
            }
            if (volPct !== null && volPct >= 150) {
                badges.push('vol');
            }
            if (w52Pos >= 99) {
                badges.push('52h');
        ...
        const indices: IndexData[] = INDEX_POOL.filter((i) => idxRaw[i.sym])
            .map(({ sym, label }) => {
                const d = idxRaw[sym];
