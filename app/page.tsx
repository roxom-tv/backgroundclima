...
export default function Home() {
    ...
    const renderPositionedSponsors = (slide: Slide) => {
        if (!settings.show_sponsors || !slide.show_sponsor) {
            return null;
        }

        const positions: SponsorPosition[] = [
            'top_left',
            'top_right',
            'bottom_left',
            'bottom_right',
    ...
    useEffect(() => {
        ...
        prefetchMarketData().catch((err) =>
            console.warn('Market slide warmup prefetch failed:', err),
        );
    }, []);
    ...
    useEffect(() => {
        ...
        prefetchDebtData().catch((err) => console.warn('Debt warmup prefetch failed:', err));
    }, [hasDebtSlides]);
    ...
    useEffect(() => {
        ...
        const timeoutId = setTimeout(() => {
            ...
            setShowTransition(
                !isNextYouTube && (transitionEffect === 'fade' || transitionEffect === 'slide'),
            );
            ...
            setTimeout(() => {
                ...
                const hideDelay = nextSlide?.type === 'event' ? 900 : 400;
                setTimeout(
                    () => {
        ...
        return (
            <main className="h-screen w-screen overflow-hidden relative bg-black flex items-center justify-center p-6">
                <div className="w-full max-w-3xl border border-red-500/60 bg-[#0a0a0a] rounded-xl p-6 md:p-8 shadow-[0_0_40px_rgba(239,68,68,0.15)]">
                    <div className="text-red-500 text-2xl md:text-3xl font-bold tracking-wider">
                        CONFIGURATION ERROR
                    </div>
                    ...
                    {errorInfo?.hint ? (
                        <div className="mt-4 bg-yellow-500/10 border border-yellow-400/30 rounded-md p-3">
                            <div className="text-yellow-300 uppercase tracking-wide text-xs mb-1">
                                Hint
                            </div>
                    ...
                    <div className="mt-6 flex items-center gap-3">
                        ...
                        <div className="text-gray-400 text-sm">
                            Reintenta la carga de configuracion sin recargar la pagina.
                        </div>
                    </div>
                </div>
            </main>
                    ...
                    <>
                        ...
                        <div className="bottom-info-bar">
                            ...
                            <SponsorDisplay
                                ...
                                sponsors={sponsors}
                                visible={
                                    settings.show_sponsors && (currentSlide.show_sponsor ?? true)
                                }
                            />
                        </div>
