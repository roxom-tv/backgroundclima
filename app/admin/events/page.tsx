...
export default function EventsPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'active' | 'inactive' | 'upcoming' | 'past'>('all');
  ...
  const handleFormSubmit = useCallback(async (data: CalendarEventInsert) => {
  ...
  const handleToggleActive = useCallback(async (event: CalendarEvent) => {
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-shrink-0 pb-4">
        <div className="flex items-center justify-between mb-4 border-b-2 border-[#00ff00] pb-3">
          <div>
            <h1 className="text-2xl font-mono font-bold text-white uppercase tracking-wider">CALENDAR EVENTS</h1>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 bg-[#0a0a0a] p-3 border-2 border-[#00ff00]">
          <div className="flex gap-4 text-xs font-mono">
            ...
            <span className="text-[#888]">
              ACTIVE: <span className="text-[#00ff00] font-bold">{events.filter(e => e.is_active).length}</span>
            </span>
            <span className="text-[#888]">
              UPCOMING: <span className="text-[#00ff00] font-bold">{events.filter(e => e.start_date >= today).length}</span>
            </span>
          </div>
        ...
        {isLoading && !events.length ? (
          ...
          <div className="text-center py-12 bg-gray-800 rounded-lg border border-gray-700">
            ...
            <p className="text-gray-400 mb-4">
              {filter !== 'all' ? 'Try changing the filter' : 'Add your first event to get started'}
            </p>
            {filter === 'all' && (
              <button
                onClick={handleAddNew}
                            ...
                            <div className="flex items-center p-3 gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  ...
                                  {isPast && (
                                    <span className="text-xs px-2 py-0.5 bg-gray-600 text-gray-300 rounded">Past</span>
                                  )}
                                </div>
                              ...
                              <div className="flex items-center gap-1">
                                <button
                                  ...
                                  title={event.is_active ? 'Active - Click to disable' : 'Inactive - Click to enable'}
                                >
                                  {event.is_active ? '✓' : '○'}
                                </button>
                                <button
          ...
          <div className="bg-[#0a0a0a] w-full max-w-2xl max-h-[90vh] overflow-y-auto border-2 border-[#00ff00]">
            <div className="sticky top-0 bg-[#0a0a0a] p-4 border-b-2 border-[#00ff00] flex items-center justify-between">
              <h2 className="text-lg font-mono font-semibold text-white uppercase tracking-wider">
                {editingEvent ? `EDIT: ${editingEvent.title.toUpperCase()}` : 'ADD NEW EVENT'}
              </h2>
              <button
