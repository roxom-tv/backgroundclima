...
async function fetchMetals(
    ...
    try {
        ...
        if (!json.metals || typeof json.metals !== 'object') {
            console.error(
                "Metals.dev API response missing 'metals' object. Full response:",
        ...
        const goldUsd =
            goldPrice != null && !isNaN(Number(goldPrice)) && Number(goldPrice) > 0
                ...
                : 0;
        const silverUsd =
            silverPrice != null && !isNaN(Number(silverPrice)) && Number(silverPrice) > 0
    ...
    try {
        ...
        if (wtiResponse.status === 'fulfilled' && wtiResponse.value.ok) {
            ...
            if (wtiJson.status === 'success' && wtiJson.data) {
                ...
                wtiPrice = parseFloat(
                    String(wtiData.price || wtiData.spot || wtiData.value || '0'),
                );
        ...
        if (brentResponse.status === 'fulfilled' && brentResponse.value.ok) {
            ...
            if (brentJson.status === 'success' && brentJson.data) {
                // Cuando se usa by_code, los datos vienen directamente en data, no en data.CODE
                const brentData =
                    brentJson.data.BRENT_USD ||
                    brentJson.data.BRENT_CRUDE_USD ||
                    brentJson.data.BRENT ||
                    brentJson.data;
                brentPrice = parseFloat(
                    String(brentData.price || brentData.spot || brentData.value || '0'),
                );
            ...
            const errorText =
                brentResponse.status === 'fulfilled'
                ...
                try {
                    ...
                    if (fallbackResponse.ok) {
                        ...
                        if (fallbackJson.status === 'success' && fallbackJson.data) {
                            ...
                            const brentData =
                                fallbackJson.data.BRENT_CRUDE_USD || fallbackJson.data;
                            brentPrice = parseFloat(
                                String(brentData.price || brentData.spot || brentData.value || '0'),
                            );
...
async function fetchPyth(): Promise<{
    ...
    try {
        ...
        if (!data.parsed || !Array.isArray(data.parsed)) {
            console.error(
                'Pyth response missing parsed array. Keys:',
...
export async function GET() {
    ...
    try {
        ...
        const copper =
            copperData.status === 'fulfilled'
                ? copperData.value
                : { usd: 0, change24hPct: null as number | null };

        const copperStatus =
            copperData.status === 'rejected' ? 'fetch_failed' : copper.usd > 0 ? 'ok' : 'no_data';
        ...
        if ((oil.wti.usd === 0 && oil.brent.usd === 0) || pythData.status === 'rejected') {
            if (oilCache && now - oilCache.timestamp < OIL_CACHE_DURATION * 2) {
                ...
                oil.brent = {
                    usd: cachedOil.brent.usd,
                    change24hPct: cachedOil.brent.change24hPct,
                };
            }
        }
        ...
        // FX fallback
        if (
            (fx.EUR.usdPerUnit === 0 && fx.JPY.usdPerUnit === 0 && fx.GBP.usdPerUnit === 0) ||
