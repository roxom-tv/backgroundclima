...
export default function StrcSlide() {
  ...
  const display = useMemo(() => {
    ...
    const atmBtcVal = isActive ? (todayDiv!.btc ?? toBtc(todayDiv!.usd, b)) : toBtc(metrics.monthlyDiv, b);
    ...
    const stats: StatItem[] = [
      { l: 'Par Value', v: BTC_FMT(toBtc(metrics.parValue, b)), u: USD_FMT(metrics.parValue), c: 'gold' },
      ...
      { l: 'Market Cap', v: (metrics.marketCap / b).toFixed(0) + ' BTC', u: USD_FMT(metrics.marketCap, 0) },
    ...
    return {
      priceBtc: BTC_FMT(s), priceUsd: USD_FMT(strc.price), changeBtc: `${sign}${diff.toFixed(8)} ₿`,
