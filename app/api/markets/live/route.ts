...
export async function GET() {
  try {
    ...
    if (shouldRefreshSummaries && provisionalTopSymbols.length > 0) {
      const summaryResults = await Promise.allSettled(
        provisionalTopSymbols.map((sym) => withRetry(() => fetchSummary(sym), 2, 600))
      );

      const nextSummaries: Record<string, SummaryRaw> = { ...staleCache };
      provisionalTopSymbols.forEach((sym, i) => {
        const result = summaryResults[i];
        if (result.status === 'fulfilled') {
          nextSummaries[sym] = result.value;
        } else {
    ...
    allRankedTickers.sort((a, b) => Math.abs(b.chgPct) - Math.abs(a.chgPct) || b.volPct - a.volPct);
