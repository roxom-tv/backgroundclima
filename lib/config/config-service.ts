...
export async function getConfigSnapshot(): Promise<AppConfigSnapshot> {
  ...
  const [slidesRes, settingsRes, sponsorsRes, eventsRes] = await Promise.all([
    ...
    from('sponsors').select('*').eq('is_active', true).order('order_index', { ascending: true }),
    from('events').select('*').eq('is_active', true).order('start_date', { ascending: true }),
  ]);

  if (slidesRes.error) throw new Error(`Failed to fetch slides: ${slidesRes.error.message}`);
  if (settingsRes.error && settingsRes.error.code !== 'PGRST116') {
    throw new Error(`Failed to fetch settings: ${settingsRes.error.message}`);
  }
  if (sponsorsRes.error) throw new Error(`Failed to fetch sponsors: ${sponsorsRes.error.message}`);
...
export async function getConfigVersion(): Promise<string> {
  ...
  const [slidesRes, settingsRes, sponsorsRes, eventsRes] = await Promise.all([
    from('slides').select('updated_at').eq('is_active', true).order('updated_at', { ascending: false }).limit(1),
    from('settings').select('updated_at').eq('key', 'global').limit(1),
    from('sponsors').select('updated_at').eq('is_active', true).order('updated_at', { ascending: false }).limit(1),
    from('events').select('updated_at').eq('is_active', true).order('updated_at', { ascending: false }).limit(1),
  ]);

  if (slidesRes.error) throw new Error(`Failed to fetch config version (slides): ${slidesRes.error.message}`);
  if (settingsRes.error) throw new Error(`Failed to fetch config version (settings): ${settingsRes.error.message}`);
  if (sponsorsRes.error) throw new Error(`Failed to fetch config version (sponsors): ${sponsorsRes.error.message}`);
  if (eventsRes.error) throw new Error(`Failed to fetch config version (events): ${eventsRes.error.message}`);
