...
export default function SlidesPage() {
  ...
  const filteredSlides = slides.filter(slide => {
    if (filter === 'active') return slide.is_active;
    if (filter === 'inactive') return !slide.is_active;
    return true;
  });
  ...
  // Handle form submit
  const handleFormSubmit = useCallback(async (data: SlideInsert) => {
  ...
  // Handle delete
  const handleDelete = useCallback(async (slide: Slide) => {
  ...
  const handleToggleActive = useCallback(async (slide: Slide) => {
  ...
  const getTypeInfo = (type: string) => {
    switch (type) {
      case 'youtube': return { icon: '📺', label: 'YouTube', color: 'text-red-400' };
      case 'debt': return { icon: '💵', label: 'US Debt', color: 'text-yellow-400' };
      case 'metals': return { icon: '🥇', label: 'Metals', color: 'text-amber-400' };
      case 'fx': return { icon: '💱', label: 'FX', color: 'text-cyan-400' };
      case 'market': return { icon: '📈', label: 'Market', color: 'text-emerald-400' };
      case 'show': return { icon: '🎬', label: 'Show', color: 'text-green-400' };
      case 'event': return { icon: '📅', label: 'Event', color: 'text-purple-400' };
  ...
  const isSystemSlide = (type: string) => ['debt', 'metals', 'fx', 'market', 'strc', 'sata'].includes(type);
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-shrink-0 pb-4">
        <div className="flex items-center justify-between mb-4 border-b-2 border-[#00ff00] pb-3">
          <div>
            <h1 className="text-2xl font-mono font-bold text-white uppercase tracking-wider">SLIDES</h1>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 bg-[#0a0a0a] p-3 border-2 border-[#00ff00]">
          <div className="flex gap-4 text-xs font-mono">
            ...
            <span className="text-[#888]">
              ACTIVE: <span className="text-[#00ff00] font-bold">{slides.filter(s => s.is_active).length}</span>
            </span>
            <span className="text-[#888]">
              DURATION: <span className="text-white font-bold">
                {Math.floor(slides.filter(s => s.is_active).reduce((acc, s) => acc + s.duration_seconds, 0) / 60)}m {slides.filter(s => s.is_active).reduce((acc, s) => acc + s.duration_seconds, 0) % 60}s
              </span>
            </span>
          </div>
        ...
        {isLoading && !slides.length ? (
          ...
          <div className="text-center py-12 bg-[#0a0a0a] border-2 border-[#00ff00]">
            ...
            <h3 className="text-lg font-mono font-medium text-white mb-2 uppercase tracking-wider">
              {filter !== 'all' ? `NO ${filter.toUpperCase()} SLIDES` : 'NO SLIDES YET'}
            </h3>
            <p className="text-[#888] mb-4 text-xs font-mono uppercase tracking-wider">
              {filter !== 'all' ? 'TRY CHANGING THE FILTER' : 'ADD YOUR FIRST SLIDE TO GET STARTED'}
            </p>
            {filter === 'all' && (
              <button
                onClick={handleAddNew}
                        ...
                        {(provided, snapshot) => (
                          ...
                          >
                            <div className="flex items-center p-3 gap-3">
                              ...
                              {/* Type Icon */}
                              <span className="text-xl" title={typeInfo.label}>{typeInfo.icon}</span>
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  <span className="text-white font-medium truncate">{slide.name}</span>
                                  {slide.country && (
                                    <span className="text-gray-500 text-sm truncate">({slide.country})</span>
                                  )}
                                  ...
                                  {slide.type === 'event' && slide.start_date && (
                                    <span 
                                      className="px-2 py-0.5 rounded text-xs font-medium"
                                      style={{ backgroundColor: slide.color || '#3B82F6', color: 'white' }}
                                    >
                                      {formatDate(slide.start_date)}
                                      {slide.end_date && slide.end_date !== slide.start_date && ` - ${formatDate(slide.end_date)}`}
                                    </span>
                                  )}
                                </div>
                                <div className="flex items-center gap-3 text-xs text-gray-500">
                                  ...
                                  {slide.show_weather && slide.type === 'youtube' && (
                                    <span className="text-blue-400" title="Weather enabled">🌤️</span>
                                  )}
                                  {slide.show_sponsor && slide.type !== 'debt' && (
                                    <span className="text-yellow-400" title="Sponsor enabled">💰</span>
                                  )}
                                  {slide.type === 'debt' && (
                                    <span className="text-gray-400">Treasury API data</span>
                                  )}
                                  {slide.type === 'event' && slide.description && (
                                    <span className="hidden md:inline truncate max-w-[200px] text-gray-400">{slide.description}</span>
                                  )}
                                  {slide.type === 'show' && slide.host_name && (
                                    <span className="text-green-400">📺 {slide.host_name}</span>
                                  )}
                                  {slide.type === 'news' && slide.headline && (
                                    <span className="text-orange-400 hidden md:inline truncate max-w-[200px]">📰 {slide.headline}</span>
                                  )}
                                  {slide.type === 'youtube' && slide.timezone && (
                                    <span className="hidden md:inline truncate max-w-[150px]">{slide.timezone}</span>
                                  )}
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <button
                                  ...
                                  title={slide.is_active ? 'Active - Click to disable' : 'Inactive - Click to enable'}
                                >
                                  {slide.is_active ? '✓' : '○'}
                                </button>
                                ...
                                {isSystemSlide(slide.type) ? (
                                  <>
                                    ...
                                    {slides.filter(s => s.type === slide.type).length > 1 && (
                                      <button
          ...
          <div className="bg-[#0a0a0a] w-full max-w-2xl max-h-[90vh] overflow-y-auto border-2 border-[#00ff00]">
            <div className="sticky top-0 bg-[#0a0a0a] p-4 border-b-2 border-[#00ff00] flex items-center justify-between">
              <h2 className="text-lg font-mono font-semibold text-white uppercase tracking-wider">
                {editingSlide ? `EDIT: ${editingSlide.name.toUpperCase()}` : 'ADD NEW SLIDE'}
              </h2>
              <button
          ...
          <div className="bg-[#0a0a0a] w-full max-w-md border-2 border-[#00ff00]">
            <div className="p-4 border-b-2 border-[#00ff00] flex items-center justify-between">
              <h2 className="text-lg font-mono font-semibold text-white uppercase tracking-wider flex items-center gap-2">
                <span className="opacity-70">{getTypeInfo(editingDebtSlide.type).icon}</span>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-center justify-between border-b border-[#1a1a1a] pb-3">
                <label className="text-white font-mono text-xs uppercase tracking-wider">ACTIVE</label>
                <button
                  onClick={() => setEditingDebtSlide({ ...editingDebtSlide, is_active: !editingDebtSlide.is_active })}
                  className={`w-14 h-7 transition-colors border-2 ${
                    editingDebtSlide.is_active ? 'bg-[#00ff00] border-[#00ff00]' : 'bg-[#1a1a1a] border-[#333]'
                  }`}
                >
                  <div className={`w-5 h-5 bg-black transition-transform mx-0.5 ${
                    editingDebtSlide.is_active ? 'translate-x-7' : 'translate-x-0'
                  }`} />
                </button>
              </div>
              ...
              {/* Sponsor Toggle */}
              <div className="flex items-center justify-between border-b border-[#1a1a1a] pb-3">
                <label className="text-white font-mono text-xs uppercase tracking-wider">SHOW SPONSOR</label>
                <button
                  onClick={() => setEditingDebtSlide({ ...editingDebtSlide, show_sponsor: !editingDebtSlide.show_sponsor })}
                  className={`w-14 h-7 transition-colors border-2 ${
                    editingDebtSlide.show_sponsor ? 'bg-[#ffff00] border-[#ffff00]' : 'bg-[#1a1a1a] border-[#333]'
                  }`}
                >
                  <div className={`w-5 h-5 bg-black transition-transform mx-0.5 ${
                    editingDebtSlide.show_sponsor ? 'translate-x-7' : 'translate-x-0'
                  }`} />
                </button>
              </div>
              ...
              <div className="border-b border-[#1a1a1a] pb-3">
                <label className="text-white font-mono text-xs uppercase tracking-wider block mb-2">DURATION (SECONDS)</label>
                <input
                  type="number"
                  value={debtDuration}
                  onChange={(e) => setDebtDuration(Math.max(5, parseInt(e.target.value) || 30))}
                  min={5}
                  max={300}
                  className="w-full bg-[#1a1a1a] border-2 border-[#00ff00] px-4 py-2 text-white font-mono focus:outline-none focus:border-[#00cc00]"
                />
                <p className="text-[#888] text-xs font-mono mt-1 uppercase tracking-wider">HOW LONG TO DISPLAY (5-300 SECONDS)</p>
              </div>
              ...
              <div className="bg-[#1a1a1a] p-4 text-xs font-mono text-[#888] border-l-2 border-[#00ff00]">
                <p className="flex items-center gap-2">
                  ...
                  <span>
                    ...
                    {editingDebtSlide.type === 'market' && 'CONTENT IS FETCHED FROM THE LIVE MARKETS API (/API/MARKETS/LIVE).'}
